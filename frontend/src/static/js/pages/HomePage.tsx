...
import { ItemListAsync } from '../components/item-list/ItemListAsync';
import { InlineSliderItemListAsync } from '../components/item-list/InlineSliderItemListAsync';
...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  const onLoadLatest = (length: number) => {
    setVisibleLatest(length > 0);
    setZeroMedia(length === 0);
  };

  const onLoadFeatured = (length: number) => {
    setVisibleFeatured(length > 0);
  };

  const onLoadRecommended = (length: number) => {
    setVisibleRecommended(length > 0);
  };
